...
- Keep source provenance visible when web information is used.
- You are STORM regardless of the underlying model provider.`
...
function directWebAnswer(web) {
  ...
  return { text: `${snippet}${snippet.endsWith('.') ? '' : '.'}`, mode: 'web-direct', provider: first.provider || 'web' }
}
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    return send(res, 200, {
      ...
      sources: sourceMeta(),
      architecture: 'icm-direct',
      modelOrder: ['openai', 'vercel-gateway', 'openrouter-free', 'anthropic', 'xai'],
  ...
  const messages = [
    ...
    ...history.slice(-12).map((item) => ({ role: item.role === 'storm' ? 'assistant' : 'user', content: String(item.text || '') })),
