const MODEL_PROVIDERS = {
  openrouterFree: {
    ...
    headers: (key) => ({
      ...
      'HTTP-Referer': process.env.STORM_APP_URL || 'https://top-lite.vercel.app',
...
export async function runModel(messages) {
  const preferred = (process.env.STORM_PROVIDER || 'openai').toLowerCase()
  const aliases = { openrouter: 'openrouterFree', free: 'openrouterFree' }
  const normalizedPreferred = aliases[preferred] || preferred
  const order = [normalizedPreferred, 'openai', 'gateway', 'openrouterFree', 'anthropic', 'xai'].filter((v, i, a) => a.indexOf(v) === i)
  const errors = []
